// FROM: https://github.com/tailscale/tailscale/blob/v1.34.2/net/socks5/socks5.go
// modifications: see DEL:
...
func (s *Server) Serve(l net.Listener) error {
	...
	for {
		...
		go func() {
			defer c.Close()
			conn := &Socks5Conn{clientConn: c, srv: s}
			err := conn.Run()
			if err != nil {
...
// Socks5Conn is a SOCKS5 connection for client to reach
// server.
...
func (c *Socks5Conn) handleRequest() error {
	...
	// DEL: ctx, cancel := context.WithTimeout(context.Background(), 5*time.Second)
	...
	res := &response{
		reply:        success,
		bindAddrType: c.request.destAddrType,
		bindAddr:     c.request.destination,
		bindPort:     c.request.port,
	}
	buf, err := res.marshal()
	if err != nil {
		res = &response{reply: generalFailure}
